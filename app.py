...
# -----------------------------------------------------------------------------
    ...
    with s_col1:
        def on_scenario_change_app():
            sc = st.session_state.get("scenario_radio_app", "")
            if "Conservative" in sc:
                st.session_state["rev_lift_app"] = 0.8
            elif "Aggressive" in sc:
                st.session_state["rev_lift_app"] = 2.5
            else:
                st.session_state["rev_lift_app"] = 1.5

        if "rev_lift_app" not in st.session_state:
            st.session_state["rev_lift_app"] = 1.5
        ...
        selected_scenario = st.radio(
            ...
            on_change=on_scenario_change_app,
        )
        ...
        expected_lift = st.slider("Projected CVR Lift from Roadmap Bet (%)", min_value=0.2, max_value=5.0, step=0.1, key="rev_lift_app")
